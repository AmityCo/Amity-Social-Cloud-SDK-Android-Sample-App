...
import com.ekoapp.sample.chatfeature.intent.OpenChangeMetadataIntent
import com.ekoapp.sample.chatfeature.userlist.UserListActivity
...
class ChannelListActivity : AppCompatActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        if (id == R.id.action_register) {
            ...
            val defPreference = defaultPreference(this)
            showDialog(R.string.change_api_key, "", defPreference.ekoApiKey.toString(), false, { dialog, input ->
                val newApiKey = input.toString()
                defPreference.ekoApiKey = newApiKey
    ...
    private fun getChannelsLiveData(): LiveData<PagedList<EkoChannel>> {
        ...
        val types = FluentIterable.from(defaultPreference(this).channelTypeOptions)
                .transform {
                    EkoChannel.Type.fromJson(it)
                }.toSet()

        val filter = EkoChannelFilter.fromApiKey(defaultPreference(this).channelMembershipOption)
        val includingTags = defaultPreference(this).includingChannelTags?.let { EkoTags(it) }
        val excludingTags = defaultPreference(this).excludingChannelTags?.let { EkoTags(it) }
    ...
    private fun displayQueryOptions() {
        val types = "Types: " + Joiner.on(",").join(defaultPreference(this).channelTypeOptions)
        val filter = "\nMembership: " + defaultPreference(this).channelMembershipOption.toString()
        val includeTags = "\nincludeTags: " + Joiner.on(",").join(defaultPreference(this).includingChannelTags)
        val excludingTags = "\nexcludingTags: " + Joiner.on(",").join(defaultPreference(this).excludingChannelTags)
