...
import kotlinx.android.synthetic.main.bottom_sheet_users.*

class ConversationWithUsersBottomSheetFragment(private val lifecycleOwner: LifecycleOwner, private val viewModel: ChannelsViewModel) : BottomSheetDialogFragment() {

    private lateinit var adapter: ConversationWithUserAdapter
    private var fragmentView: View? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        fragmentView = inflater.inflate(R.layout.bottom_sheet_users, container, false)
        return fragmentView
    }
    ...
    private fun Context.renderList() {
        ...
        RecyclerBuilder(context = this, recyclerView = recycler_users)
                .builder()
                .build(adapter)
        viewModel.bindUsers().observeNotNull(lifecycleOwner, adapter::submitList)
    }
}